...
static float   STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* obj, int line_start_idx, int char_idx)
{
    ...
    ImGuiContext& g = *GImGui;
    return g.Font->GetCharAdvance((ImWchar)data.Codepoint) * (g.FontSize / g.Font->FontSize);
}
...
static int  is_word_boundary_from_right(ImGuiInputTextState* obj, int idx)
{
    if (obj->Flags & ImGuiInputTextFlags_Password)
        return 0;
    unsigned int c2;
    ImGuiInputTextCharInfo data = InputTextGetCharInfo(obj, idx);
    ImTextCharFromUtf8(&c2, data.Text + data.CodepointBytes, obj->TextA.Data + obj->CurLenA);
    return idx > 0 ? (is_separator(data.Codepoint) && !is_separator(c2)) : 1;
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // FIXME: For non-readonly widgets we might be able to require that TextA == buf ? (untested) and discard undo stack if user buffer has changed.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                const int clipboard_len = (int)ImTextCountCharsFromUtf8(clipboard, NULL);
