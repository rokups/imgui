...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + g.Style.PointSizeRound), col, 12);

    float cross_extent = g.FontSize * 0.5f * 0.7071f - 1.0f;
    ImU32 cross_col = GetColorU32(ImGuiCol_Text);
    center -= ImVec2(0.5f, 0.5f);
    window->DrawList->AddLine(center + ImVec2(+cross_extent,+cross_extent), center + ImVec2(-cross_extent,-cross_extent), cross_col, g.Style.PointSizeRound);
    window->DrawList->AddLine(center + ImVec2(+cross_extent,-cross_extent), center + ImVec2(-cross_extent,+cross_extent), cross_col, g.Style.PointSizeRound);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    ImVec2 center = bb.GetCenter();
    if (hovered || held)
        window->DrawList->AddCircleFilled(center + off + ImVec2(0,-0.5f), g.FontSize * 0.5f + g.Style.PointSizeRound, bg_col, 12);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    float thickness_draw = g.Style.PointSizeRound;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            float bg_offy_dn = is_multiline ? 0.0f : 2.0f * style.PointSizeRound;
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 0.5f, cursor_screen_pos.x + style.PointSizeRound, cursor_screen_pos.y - 1.5f * style.PointSize);
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags, ImGuiDockNode* dock_node)
{
    ...
    else
    {
        ...
        window->DrawList->AddLine(ImVec2(separator_min_x, y), ImVec2(separator_max_x, y), col, g.Style.PointSizeRound);
    }
    return true;
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    columns->OffMaxX = ImMax(ImMin(max_1, max_2) - window->Pos.x, columns->OffMinX + g.Style.PointSizeRound);
    ...
    for (int n = 0; n < columns_count; n++)
    {
        ...
        float clip_x2 = IM_ROUND(window->Pos.x + GetColumnOffset(n + 1) - g.Style.PointSizeRound);
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            window->DrawList->AddLine(ImVec2(xi, y1 + g.Style.PointSizeRound), ImVec2(xi, y2), col);
        }
